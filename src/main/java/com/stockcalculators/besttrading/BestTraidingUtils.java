...
class BestTraidingUtils {
  ...
  static BestTradingResult calculateBestTradingResultInternal(
    ...
    int pricesSize = lowPrices.size();
    if (pricesSize == 0) {
      // No data at all means that no trade is possible.
      return BestTradingResult.noTrade(calculationDate);
    }
    if (lowTimes.size() != pricesSize || highPrices.size() != pricesSize || highTimes.size() != pricesSize) {
    ...
    LocalDate[] dates = DateTimeUtils.buildTradingDates(lastTradingDate, pricesSize);
    LocalTime[] lowLocalTimes = new LocalTime[pricesSize];
    LocalTime[] highLocalTimes = new LocalTime[pricesSize];
    Instant[] lowInstants = new Instant[pricesSize];
    Instant[] highInstants = new Instant[pricesSize];
    ...
    // Running maximum profit (in doubles before we cast to int) and its associated trade.
    ...
    // First, handle cross-day trades in a single pass using the classic
    // "minimum so far" technique.
    double minLowPrice = lowPrices.get(0);
    int minLowDay = 0;

    for (int sellDay = 1; sellDay < pricesSize; sellDay++) {
    ...
    // Next, consider same-day trades where low time is strictly earlier than high time.
